...
`recode()`, like `group_by()`, `summarize()`, and the other dplyr functions, quotes its input. We therefore need to tell `recode()` to evaluate `drv_recode` immediately. Let's try `!!`.
...
`!!` doesn't work because `drv_recode` is a vector. Not only do we need to immediately evaluate `drv_recode`, we also need to unpack its contents. To do so, we'll use `!!!`.
